...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, max_unique_categorical_value: float = 0.85) -> Dict[str, Any]:
    """
    ...
    for j in [i.unique for i in summary.columns]:
        if j < 2:
            count_constant_columns += 1
    flags["has_constant_columns"] = count_constant_columns > 0

    count_high_cardinality_categoricals = 0
    for j in summary.columns:
        if not(j.is_numeric) and j.unique > summary.n_rows * max_unique_categorical_value:
            count_high_cardinality_categoricals += 1
    flags["has_high_cardinality_categoricals"] = count_high_cardinality_categoricals > 0
    ...
    score -= count_constant_columns / summary.n_cols #чем больше колонок с одинаковыми значениями, тем хуже
    score -= count_high_cardinality_categoricals / summary.n_cols #чем больше категориальных колонок с большим числом уникальных значений, тем хуже
